...
static s32 __usbstorage_reset(struct ehci_hcd * ehci, usbstorage_handle *dev,int hard_reset)
{
 ...
 retry:
        if (retry >= 1){
				...
				if(__lun != 16)
					{
					if(USBStorage_MountLUN(ehci, &__usbfd, __lun) < 0)
                        goto end;
					}
        }
...
s32 USBStorage_Try_Device(struct ehci_hcd * ehci,struct ehci_device *fd)
{
		...
		try_status=-120;
		if(USBStorage_Open(ehci,&__usbfd, fd) < 0)
			return -EINVAL;
	...
	while(1)
       {
		   ...
		   #endif
           retval = USBStorage_MountLUN(ehci,&__usbfd, j);
		   #ifdef MEM_PRINT
           ...
           if(retval < 0)
				...
				if(test_max_lun)
					{
					__usbfd.max_lun = 0;
					retval = __USB_CtrlMsgTimeout(ehci,&__usbfd,
...
{
	return usbstorage_ready;
}

DISC_INTERFACE usb2mass_ops={
    .readSectors = (FN_MEDIUM_READSECTORS)&USBStorage_Read_Sectors,
    .writeSectors = (FN_MEDIUM_WRITESECTORS)&USBStorage_Write_Sectors,
    .clearStatus = (FN_MEDIUM_CLEARSTATUS)&USBStorage_True,
    .shutdown = (FN_MEDIUM_SHUTDOWN)&USBStorage_True,
    .isInserted = (FN_MEDIUM_ISINSERTED)&USBStorage_Inserted,
...
int retval=1;
	...
	if(unplug_device!=0 )
	{
		...
		if(__usbfd.usb_fd)
			if(ehci_reset_port2(__ehci, /*__usbfd.usb_fd->port*/0)>=0)
			{
				if(__usbfd.buffer != NULL)
						USB_Free(__usbfd.buffer);
				__usbfd.buffer= NULL;

				if(ehci_reset_port(__ehci, 0)>=0)
				{
					handshake_mode=1;
					if(USBStorage_Try_Device(__ehci, __usbfd.usb_fd)==0) {retval=0;unplug_device=0;}
					handshake_mode=0;
				}
			}
		ehci_msleep(100);
	...
	//ehci_usleep(100);
	  ...
	  if(!unplug_procedure())
		...
		if(retval == USBSTORAGE_ETIMEDOUT && retry>0)
		 ...
		 usb_timeout=1000*1000; // 4 seconds to wait
	    if(retval >=0)
		   retval = USBStorage_Write(__ehci, &__usbfd, __lun, sector, numSectors, buffer);
		usb_timeout=1000*1000;
		if(unplug_device!=0 ) continue;
		if(retval>=0) break;
	}
